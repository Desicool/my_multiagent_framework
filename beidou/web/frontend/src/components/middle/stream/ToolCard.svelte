<script lang="ts">
  ...
  let primitive = $derived(isPrimitive(item.name));

  // Resolve the host agent (the pinned agent owns this stream item).
  // Falls back to undefined when no agent is pinned (degenerate case).
  let callerAgent = $derived(ui.pinnedAgentId ? events.agentsById[ui.pinnedAgentId] : undefined);
...
{#if primitive}
  <PrimitiveCard {item} {callerAgent} />
{:else}
