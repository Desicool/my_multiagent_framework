<script lang="ts">
  ...
  let scope = $derived((item.input?.scope as string) ?? 'team');
...
>
  {#snippet body()}
    ...
    <div class="text-[11px] uppercase tracking-wider text-muted font-medium pt-px">scope</div>
    <div class="font-mono text-[11.5px] text-slate-200">{scope}</div>
  {/snippet}
</PCardShell>
